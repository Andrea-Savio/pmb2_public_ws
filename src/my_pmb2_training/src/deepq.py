...
if __name__ == '__main__':
    ...
    rospy.init_node('example_pmb2_maze_dqn', anonymous=True, log_level=rospy.WARN)
    #map = rospy.Subscriber("/map", OccupancyGrid, map_callback)
    # Init OpenAI_ROS ENV
    task_and_robot_environment_name = rospy.get_param('/pmb2/task_and_robot_environment_name')
    ...
    pkg_path = rospack.get_path('pmb2_openai_ros_example')
    ...
    """
    #Alpha = rospy.get_param("/pmb2/alpha")
    #Epsilon = rospy.get_param("/pmb2/epsilon")
    #Gamma = rospy.get_param("/pmb2/gamma")
    #epsilon_discount = rospy.get_param("/pmb2/epsilon_discount")
    nepisodes = rospy.get_param("/pmb2/nepisodes")
    num_steps = rospy.get_param("/pmb2/nsteps")
    #running_step = rospy.get_param("/pmb2/running_step")
    ...
    #num_steps = 20  # 15e4 11e4 number of steps to train on
    ...
    while step_count < num_steps:
        ...
        env.stats_recorder.done = None #added by user
        ...
        while not done:	
            ...
            if step_count % target_update == 0:
                ...
                #torch.save(policy_net.state_dict(), '~/pmb2_public_ws/src/my_pmb2_training/src/checkpoints/dqn-{}.pt'.format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S")))
